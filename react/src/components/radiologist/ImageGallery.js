...
import { prepareAnnotationData, loadImageFromFile } from "./backend-functions";
...
export async function saveAnnotationsToBackend(
  ...
  const response = await fetchWithAuth(
    "http://" + config.DjangoHost + ":8000/api/sauvgarderAnnotation",
    {
      method: "POST",
      headers: {
...
export function runAIAnalysis(eco_path) {
  return new Promise((resolve, reject) => {
    const token = localStorage.getItem("access_token");
    const url = `http://` + config.DjangoHost + `:8000/api/ia/eco/`;
    ...
    fetchWithAuth(url, {
      ...
      .then((data) => {
        ...
        const aiResults = flattenedBoxes.map((box, index) => {
          const [x1, y1, x2, y2, t, f] = box;

          let rounded = (t * 100).toFixed(2);

          // Ajout : nom de la classe
          let label = f === 0 ? "Benign" : "Malignant";
          ...
          return {
            ...
            h: y2 - y1,
            source: `AI$-${label}`,
            note: `Detection IA - ${label} - Confiance: ${rounded}%`,
          };
        });
...
export default function ImageGallery({
  ...
  const updateAnnotationNote = (e) => {
    const note = e.target.value;
    setAnnotationNote(note);
    if (selectedId) {
      setAnnotations((prev) =>
        prev.map((ann) => (ann.id === selectedId ? { ...ann, note } : ann))
      );
    }
  };
  ...
  const startResize = () => {
    ...
    const handleMouseMove = (e) => {
      if (overlayRef.current) {
        ...
        setAnnotations((anns) =>
          anns.map((a) =>
            a.id === selectedId ? { ...a, w: x - a.x, h: y - a.y } : a
          )
        );
      }
    };
  ...
  const handleMouseDown = (e) => {
    ...
    // Dessin avec clic gauche en mode dessin
    if (isDrawingMode && overlayRef.current) {
      const rect = overlayRef.current.getBoundingClientRect();
      const x = (e.clientX - rect.left - pan.x) / scale;
      const y = (e.clientY - rect.top - pan.y) / scale;
  ...
  const handleMouseMove = (e) => {
    ...
    // Gestion du dessin
    if (currentRect && overlayRef.current) {
      const rect = overlayRef.current.getBoundingClientRect();
      const x = (e.clientX - rect.left - pan.x) / scale;
      const y = (e.clientY - rect.top - pan.y) / scale;
    ...
    // Gestion du drag d'annotation
    if (dragId !== null && overlayRef.current) {
      const rect = overlayRef.current.getBoundingClientRect();
      const x = (e.clientX - rect.left - pan.x) / scale;
      const y = (e.clientY - rect.top - pan.y) / scale;
  ...
  const runAI = async () => {
    ...
    setIsLoading(true);
    try {
      const ecoPath = selectedImage.path.replace(
        "http://" + config.DjangoHost + ":8000/",
        ""
      );
  ...
  const loadAnnotations = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `http://` +
          config.DjangoHost +
          `:8000/api/load-annotations?image_id=${selectedImage.id}`
      );
      ...
      const boxesList = Array.isArray(data.boxes) ? data.boxes.flat() : [];

      const notesList = Array.isArray(data.etiq) ? data.etiq.flat() : [];

      const formattedAnnotations = boxesList
        .map((box, index) => {
          if (box.x !== undefined)
            return {
              ...box,
  ...
  const handleDeleteAPI = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetchWithAuth(
        "http://" + config.DjangoHost + ":8000/api/patient/supprimerimage/",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="image-gallery-container">
      ...
      <div className="main-content1" ref={scrollWrapperRef}>
        <div className="canvas-section">
            ...
            {selectedImage && (
              ...
              >
                ...
                {annotations.map((r) => {
                  ...
                  let label = "Unknown";
                  if (r.source && r.source.startsWith("AI$-")) {
                    label = r.source.split("$-")[1];
                  }

                  let baseColor, baseBg, baseLabelBg;

                  if (label === "Benign") {
                    baseColor = "rgba(39, 174, 96, 0.8)";
                    baseBg = "rgba(39, 174, 96, 0.1)";
                    baseLabelBg = "rgba(39, 174, 96, 0.8)";
                  } else {
                    baseColor = "rgba(231, 76, 60, 0.8)";
                    baseBg = "rgba(231, 76, 60, 0.1)";
                    baseLabelBg = "rgba(231, 76, 60, 0.8)";
                  }

                  const borderColor = isSelected
                    ? baseColor.replace("0.8", "0.9")
                    : baseColor;
                  const bgColor = isSelected
                    ? baseBg.replace("0.1", "0.2")
                    : baseBg;
                  const labelBgColor = isSelected
                    ? baseLabelBg.replace("0.8", "0.9")
                    : baseLabelBg;

                  const displayNote = r.note;
                ...
                {selectedId !== null && (
                  <>
                    ...
                    <div
                      ...
                      style={{ top: iconPos.top + 35, left: iconPos.left }}
                    ...
                    <div
                      className="icon resize-icon"
                      style={{ top: iconPos.top + 35, left: iconPos.left + 40 }}
                      onMouseDown={(e) => {
                        e.stopPropagation();
                        startResize();
                      }}
                      title="Redimensionner annotation"
                    >
                      ↘️
                    </div>
                  </>
          ...
          <div className="annotation-counter-container">
          ...
          {selectedId !== null && (
            <div className="annotation-info">
              <h3>Détails de l'annotation</h3>
              <p style={{ color: "black" }}>
                Ajoutez des notes ou observations pour cette région:
              </p>
              <textarea
                value={annotations.find((a) => a.id === selectedId)?.note || ""}
                onChange={updateAnnotationNote}
                placeholder="Décrivez ce que vous observez dans cette région (ex: nodule, opacité, etc.)"
              />
            </div>
          )}
        </div>
